...
namespace OsMapDownloader.Qct
{
    public class QctWriter
    {
        ...
        /// </summary>
        public QctWriter(ProgressTracker progress, GeographicalReferencingCoefficients geographicalReferencingCoefficients, Color[] palette, byte[] interpolationMatrix, QctMetadata metadata, uint width, uint height)
        {
        ...
        public async Task Build(Tile[] tiles, MapArea area, string newQctFilePath, bool shouldOverwrite, bool disableHardwareAccel, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            await GeographicalReferencingCoefficients.Write(fs, 0x60, cancellationToken);
